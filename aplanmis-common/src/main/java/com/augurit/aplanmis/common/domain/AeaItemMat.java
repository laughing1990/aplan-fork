...
public class AeaItemMat extends MindBaseNode {
    ...
    private String attDirId; // (所属文件库ID)
    private String matHolder; // (材料所属，c表示企业，u表示个人，p表示工程项目)
    private String isOfficialDoc; // (是否批复文件，0表示否，1表示是)
    private String attIsRequire; // (电子材料是否必需，0表示容缺，1表示必须)
    private String isCommon; // (是否通用材料  1 是 0 不是)
    private String reviewKeyPoints; // (审查样本)
    private String reviewSampleDoc; 
    private String reviewBasis; // (审查依据)
    private String matFromDept; // (材料来源部门)
    private String duePaperType; // (原件验收 0 无 1验 2收)
    private String dueCopyType; // (复印件验收类型 0 无 1验 2收)
    private String paperMatType; // (纸质材料类型 0无 1 A3, 2 A4 ,3 A5)
    private String zcqy; // (是否支持容缺 0 否 1是)
