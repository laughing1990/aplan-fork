...
var parallelDeclare = new Vue({
  ...
  data: function () {
    ...
    return {
      ...
      model: {
        rules: {
          // applySelOpinion: {required: true, message: "请输入意见", trigger: ["blur"]},
        },
        coreItems: [],
        parallelItems: [],
      },
  ...
  methods: {
    test: function(isApplySel,isITSel,rowData){
      if(isApplySel!==isITSel){
        rowData.rules=true
      }else {
        rowData.rules=false
      }
    ...
    isNeedIntelligence: function(){
      ...
      _that.stageList = [];
      _that.stateList = [];
      _that.model.parallelItems = [];
      _that.model.coreItems = [];
      _that.guideList=[];
    ...
    guideApplyStart: function(){
      ...
      debugger;
      if(_that.model.parallelItems.length>0){
        _that.model.parallelItems.map(function(item){
      ...
      if(_that.model.coreItems.length>0){
        _that.model.coreItems.map(function(item){
          if(item.isApplySel||item.isITSel){
            if(!item.notRegionData){
              var _itemListItem = {
                applySelOpinion: item.applySelOpinion,
    ...
    getStatusStateMats: function (stageId,draftsProjFlag) {
      ...
      request('', {
        ...
        if (res.success) {
          _that.loading = false;
          _that.model.parallelItems = res.content.parallelItemList;
          _that.model.coreItems = res.content.coreItemList;
          _that.model.coreItems.map(function (item) {
            if (item) {
              ...
              if (typeof item.rules == 'undefined' || item.rules == undefined) {
                Vue.set(item, 'rules', false);
              }
    ...
    getStatusStateBystatus: function (pData, data, index, checkFlag) {
      ...
      if (checkFlag == false) {
        ...
        // 清空情形下所对应并联事项
        for (var i = 0; i < _that.model.parallelItems.length; i++) {
          var obj = _that.model.parallelItems[i];
          if (obj && (obj.parStateId&&selStateIds.indexOf(obj.parStateId) == -1)) {
            _that.model.parallelItems.splice(i, 1);
            i--
          }
        }
        // 清空情形下所对应并行事项
        for (var i = 0; i < _that.model.coreItems.length; i++) {
          var obj = _that.model.coreItems[i];
          if (obj && (obj.parStateId&&selStateIds.indexOf(obj.parStateId) == -1)) {
            _that.model.coreItems.splice(i, 1);
            i--
          }
        }
        return false
      ...
      request('', {
        ...
        if (res.success) {
          if (checkFlag == true) {
            ...
            selStateIds = _that.getStatusListId();
            // 当前情形附带的并行事项列表
            for (var i = 0; i < _that.model.coreItems.length; i++) { // 清空情形下所对应事项
              var obj = _that.model.coreItems[i];
            ...
            for (var i = 0; i < _that.model.parallelItems.length; i++) { // 清空情形下所对应并联事项
              var obj = _that.model.parallelItems[i];
              if (obj._parStateId == 'undefined' || obj._parStateId == undefined) {
                Vue.set(obj, '_parStateId', []);
              } else {
          ...
          if (res.content.itemList.length > 0) {
            res.content.itemList.map(function (item, index) {
              ...
              _that.parallelItemsSelItem(_that.model.parallelItems, item, 'autoGetSel');
              _that.setImplementItem(item);
            });
          }
          // 当前情形附带的并联事项列表
          ...
          _that.model.coreItems = _that.unique(_that.model.coreItems.concat(res.content.coreItemList), 'stage');
          _that.model.parallelItems = _that.unique(_that.model.parallelItems.concat(res.content.itemList), 'stage');
        } else {
          _that.$message({
            message: '获取阶段下情形失败',
            type: 'error'
    ...
    itemExpandChange: function (row, expandedRows) {
      var _that = this;
      _that.model.coreItemsSelItem(expandedRows, row);
    ...
    // 事项一单清下一步 是否部门辅导
    nextOrGuidance: function(){

    },
    // 保存并下一步  获取一张表单列表
    saveAndGetOneForm: function (saveFlag,saveFlag2) {
      var _that = this;
      var sucessBtn = '', cancelBtn = '';
      _that.isGuidance=false;
      if(saveFlag=='1'){
        sucessBtn = '继续申报';
        cancelBtn = '放弃申报';
      }else {
      ...
      if(_that.stageFrontCheckFlag==false){
        confirmMsg('阶段前置检测不通过', _that.stageFrontCheckMsg, function(){
          _that.stageFrontCheckFlag = true;
          if(_that.stageId!=''&&_that.stageinstId){
            _that.getStatusStateMats(_that.stageId,true);  // 获取事项情形列表
          }else {
          ...
          _that.model.parallelItems=[];
          _that.model.coreItems=[];
        },sucessBtn,cancelBtn, 'error', true);
        return false;
      }
      ...
      for (var i = 0; i < stateSelLen; i++) {  // 并联情形id集合
        ...
        if (stateSel[i].selectAnswerId !== null || stateSel[i].selectAnswerId !== '') {
          ...
          if (typeof stateSel[i].selectAnswerId == 'object') {
            if(stateSel[i].selectAnswerId&&stateSel[i].selectAnswerId !=''){
      ...
      if(saveFlag2&&saveFlag2=='isGuidance'){
        if(!_that.isGuidanceSuccess){
          _that.declareStep = 5;
          _that.isGuidance=true;
        }else {
          _that.isGuidance=true;
        }
      }
    ...
    // 暂存阶段
    itemListTemporary: function(){
      var _that = this;
      var itemListTemporaryParamVo = {
        stageId: _that.stageId,
        themeId: _that.themeId,
        themeVerId: _that.themeVerId?_that.themeVerId:'',
      ...
      request('', {
        ...
        if(result.success){
          ...
          if(!_that.isGuidance){
            ...
            // 调用事项确认接口
            // _that.checkAndConfirmItems(result.content.applyinstId);
          }
        }else {
          if(!_that.isGuidance){
            _that.$message({
    ...
    // 事项确认
    checkAndConfirmItems: function(_applyinstId){
      // rest/user/stage/apply/getApplyIteminstConfirmDetail/{applyinstId}
      var _that = this;
      request('', {
        url: ctx + 'rest/user/stage/apply/getApplyIteminstConfirmDetail/'+_applyinstId,
        type: 'get',
      }, function (result) {
        if(result.success){
          ...
          _that.$message({
            message: result.message?result.message:'事项确认加载失败',
            type: 'error'
          });
        }
      }, function (msg) {
    ...
    matListTemporary: function(){
      ...
      var matListStageTemporaryParamVo = {
        ...
        applyinstId: _that.parallelApplyinstId,
        itemListTemporaryParamVo: itemListTemporaryParamVo,
      };
      request('', {
        url: ctx + 'rest/apply/common/matList/stage/temporary',
        type: 'post',
        ContentType: 'application/json',
