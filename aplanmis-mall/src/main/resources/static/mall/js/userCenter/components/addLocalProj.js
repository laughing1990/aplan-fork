...
var module1 = new Vue({
  ...
  methods: {
    ...
    // 根据当前登录身份获取单位信息
    fetchUnitBaseInfo: function () {
      var ts = this,
        _url = ctx + 'rest/apply/common/applyObject';
      ts.mloading = true;
      request('', {
        url: _url,
        type: 'get',
        ...
        ts.mloading = false;
        if (res.success) {
          ts.handelUnitBaseInfo(res.content)
        } else {
          return ts.apiMessage(res.message, 'error')
        }
      }, function () {
    ...
    // 根据获取到的数据初始化
    handelUnitBaseInfo: function (obj) {
      this.unitInfoData = obj;
      this.unitSelectedLinkman = obj.linkmanInfoList[0] || {};
      this.projInfoForm.linkmanInfoId = this.unitSelectedLinkman.linkmanInfoId;
      // console.log(this.unitSelectedLinkman)
    },
    // 单位联系人中下拉列表中选中一个联系人
    linkmanItemSelected: function (obj) {
      // console.log(obj)
      var allList = this.unitInfoData.linkmanInfoList;
      for (var i = 0, ln = allList.length; i < ln; i++) {
        if (allList[i].linkmanInfoId === obj) {
          this.unitSelectedLinkman = allList[i];
          break;
        }
      }
    },
