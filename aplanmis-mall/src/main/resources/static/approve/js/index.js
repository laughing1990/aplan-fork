...
// *********** 意见弹框参数 start *********************
  ...
  methods: {
    ...
    initForms: function () {
      ...
      //   if (u.labelId == 4) { tmpIndex = i}
      // })
      // if (vm.hasSupply!=1) {vm.lTabsData.splice(tmpIndex, 1);}
      // 特殊程序
      var tmpIndex = -1;
      vm.lTabsData.forEach(function (u, i) {
        if (u.labelId == 5) {
...
//==========================================================工程建设 相关审批按钮 start ========
    ...
    changeMutiMan: function (row) {
      ...
      var index = this.mutiCheckedNames.indexOf(name);
      if (index != -1) {
        this.mutiCheckedMan.push(row);
      } else {
        this.mutiCheckedMan.splice(index, 1);
      }
    },
    //下一环节选择事件，需要切换展示信息
      ...
      if (vm.isMultiFlow) {
        ...
        vm.mutiCheckedMan.forEach(function (u) {
          sendObj.sendConfigs.push({
            assignees: u.defaultSendAssignees,
...
//----------- 制证 ---------------------
    ...
    // 组装补正两个表格的数据
    packageBzData: function (data) {
      var submitData = data.submittedMats;
      var correctData = data.matCorrectDtos;
      vm.bzSubmitData = splitData(submitData);
      vm.bzCorrectData = splitData(correctData, 1);
      
      function splitData(arr, flag) {
        var tmp = [];
