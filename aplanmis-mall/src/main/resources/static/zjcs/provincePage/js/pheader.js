...
var headerVm = new Vue({
  ...
  data: function () {
    return {
      ...
      // 切换城市相关
      isShowCityPickBox: false,
      // 自治区
      regionList: [
        {name: '巴音郭楞蒙古自治州', url: 'http://218.84.184.50:8084/aplanmis-mall/supermarket/main/index.html'},
        {name: '塔城地区', url: 'http://60.13.251.82:8084/aplanmis-mall/supermarket/main/index.html'},
      ],
    }
  },
  mounted: function () {
   var ts  = this;
   window.onscroll = function () { // 定义窗口大小变更通知事件
     ts.isScroll = document.body.scrollHeight > (window.innerHeight ||document.documentElement.clientHeight);
      if (!ts.isScroll) {
  ...
  methods: {
    ...
    login: function () {
     location.href = ctx + 'supermarket/main/login.html';
    },
    init: function () {
    ...
    switchUnit: function (x, y, unitInfos) {
      ...
      $(window.document.body).append(_unitDropBox);
    ...
    showImAndOwnerSelectDialog: function () {
      var ts = this,
      _parentWin = $(window.document.body),    //父窗口body
      _sendData = {};
      var _dialogBox = `
    ...
    sendUnitSelectToApi: function(data){
      ...
      request('', {
        ...
        if (res.success) {
          ...
          window.location.href = res.content.isOwner == "1" ? ctx + "/supermarket/main/ownerCenter.html" : ctx + "/supermarket/main/agentCenter.html";
    ...
    switchUnitFetchList: function ($event) {
      ...
      _scrollX = window.document.documentElement.scrollLeft ||window.document.body.scrollLeft;
    ...
    jumpToPeopleCenter: function(){
      var _curLoginInfo = {};
        _curLoginInfo = JSON.parse(localStorage.getItem('loginInfo'));
        window.location.href = _curLoginInfo.isOwner == "1" 
    ...
    // 城市切换
    showCityPick: function(){
      console.log( this.isShowCityPickBox)
      this.isShowCityPickBox = !this.isShowCityPickBox;
