...
joint.shapes.bpmn = {}, joint.shapes.bpmn.icons = {
    ...
    initialize: function () {
        ...
        a.attr({".sub-process": {visibility: b ? "visible" : "hidden", "data-sub-process": b || ""}})
    }
    ...
    onSizeChange: function (a, b) {
        a.attr(".stroke", {d: a.getStrokePathData(b.width, b.height, a.get("wingLength"))})
    },
    ...
    defaults: joint.util.deepSupplement({
        ...
        attrs: {
            ...
            ".blackbox-label": {text: "Black Box", "text-anchor": "middle", transform: "translate(0,-7)"},
            ".blackbox-label > tspan": {dx: "50%", dy: "50%"}
        }
    }, joint.dia.Element.prototype.defaults)
    ...
    options: {headerWidth: 20}, initialize: function () {
        ...
        var f = this.options.headerWidth, g = e + "/sublanes/", h = this.calculateRatios(a, d);
        joint.util.toArray(a).reduce(function (a, d, e) {
            var i = "lane" + this.index, j = "." + i + " .lane-body", k = "." + i + " .lane-header",
                l = "." + i + " .lane-label";
            d.name && (i += " " + d.name);
            var m = g + e,
                n = this.laneMarkup.clone().addClass(i).attr({"data-lane-path": m, "data-lane-index": this.index});
            this.$lanes.append(n.node);
            var o = h[e], p = void 0 === d.headerWidth ? f : d.headerWidth, q = c + a;
    ...
    layoutAndUpdate: function () {
        var a = this.model.get("participants") || [], b = a.length, c = this.options.participantHeight,
            d = this.model.get("size").height, e = Math.max(0, d - c * b), f = 0,
            g = this.model.get("initiatingParticipant"),
            h = Math.max(joint.util.isNumber(g) ? Math.abs(g) : a.indexOf(g), 0), i = Math.min(h, b - 2);
        ...
        }, this.participantsAttrs[".content"] = {"ref-y": c * k + e / 2};
