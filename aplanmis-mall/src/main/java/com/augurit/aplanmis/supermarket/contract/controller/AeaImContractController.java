...
public class AeaImContractController {
    ...
    public ContentRestResult postContract(@RequestParam(name = "file",required = false) List<MultipartFile> files, @Valid AeaImContract aeaImContract,
        ...
        try {
            if (StringUtils.isNotBlank(aeaImContract.getContractId())){
                ...
                aeaImContractService.saveAeaImContract(aeaImContract,files,request);
            }
    ...
    public ContentRestResult<AeaImContract> confirmContractByContractId(@Valid AeaImContract aeaImContract,BindingResult bindingResult, HttpServletRequest request) throws Exception {
//        String errorMsg = BusinessUtils.checkBindingResult(bindingResult);
//
//        if(StringUtils.isNotBlank(errorMsg)){
//            return new ContentRestResult(false,null,errorMsg);
//        }
        try{
            ...
            if (loginInfoVo != null && aeaImContract!=null){
                if (loginInfoVo.getIsOwner().equals(aeaImContract.getIsOwnerUpload())) {
                    return new ContentRestResult(false, "只能由" + ("1".equals(loginInfoVo.getIsOwner())? "中介" : "业主" )+ "确认");
                }
