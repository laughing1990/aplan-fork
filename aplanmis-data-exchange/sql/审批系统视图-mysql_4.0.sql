...
(SELECT psi.STAGE_ITEM_ID AS DFSJZJ, '000000' AS XZQHDM, apt_123.THEME_CODE AS SPLCBM, aptv_123.VER_NUM AS SPLCBBH
	...
	, CASE
		...
		OR INSTR(aib_123.XKDX, ',') > 0 THEN '6'
		ELSE aib_123.XKDX
	END AS SQDX
	, CASE
	...
	, CASE
		WHEN aib_123.IS_CATALOG = '1' THEN NULL
		ELSE (
			SELECT ORG_CODE
			FROM OPU_OM_ORG
	...
	END AS SPBMBM
	, CASE
		WHEN aib_123.IS_CATALOG = '1' THEN
			CASE
				WHEN length(trim(aib_123.GUIDE_ORG_NAME)) > 0 THEN aib_123.GUIDE_ORG_NAME
				ELSE NULL
			END
		ELSE (
			SELECT ORG_NAME
	...
	END AS SPBMMC, aib_123.IS_MILESTONE_ITEM AS SFLCBSX, 1 AS SJYXBS
	, CASE
		WHEN aptv_123.MODIFY_TIME IS NOT NULL THEN aptv_123.MODIFY_TIME
		ELSE aptv_123.CREATE_TIME
	END AS MODIFY_TIME
FROM AEA_PAR_STAGE_ITEM psi
	INNER JOIN AEA_ITEM_BASIC aib_123 ON psi.ITEM_VER_ID = aib_123.ITEM_VER_ID
...
UNION ALL
(SELECT iteminst.ITEMINST_ID AS DFSJZJ, '000000' AS XZQHDM, apt_123.THEME_CODE AS SPLCBM, aptv_123.VER_NUM AS SPLCBBH
	, CASE
		WHEN stage.IS_NODE IN ('2', '3') THEN 5
		ELSE stage.SORT_NO
	END AS SPJDXH, aib_123.ITEM_CODE AS SPSXBM, aiv_123.VER_NUM AS SPSXBBH, aib_123.ITEM_NAME AS SPSXMC, aib_123.STANDARD_ITEM_CODE AS DYBZSPSXBM
	...
	, CASE
		WHEN length(trim(aib_123.SEND_RESULT_MODE)) > 0 THEN aib_123.SEND_RESULT_MODE
		ELSE '2,3'
	END AS BLJGSDFS, asti.TIME_LIMIT AS SPSXBLSX
	, (
		SELECT ORG_CODE
		FROM OPU_OM_ORG
		WHERE aib_123.ORG_ID = OPU_OM_ORG.ORG_ID
...
UNION ALL
(SELECT iteminst.ITEMINST_ID AS DFSJZJ, '000000' AS XZQHDM, apt_123.THEME_CODE AS SPLCBM, NULL AS SPLCBBH, NULL AS SPJDXH
	, aib_123.ITEM_CODE AS SPSXBM, aiv_123.VER_NUM AS SPSXBBH, aib_123.ITEM_NAME AS SPSXMC, aib_123.STANDARD_ITEM_CODE AS DYBZSPSXBM, aib_123.SFSXGZCNZ AS SFSXGZCNZ
	, CASE aib_123.ITEM_PROPERTY
		WHEN 'LBJ' THEN '3'
		WHEN 'DFJ' THEN '5'
		WHEN 'DBJ' THEN '6'
...
(SELECT ahai123.APPLYINST_PROJ_ID AS DFSJZJ, '000000' AS XZQHDM
	...
	END AS SBSJ, apt.THEME_CODE AS SPLCBM, ptv.VER_NUM AS SPLCBBH, 1 AS SJYXBS, ahai123.CREATE_TIME AS MODIFY_TIME
...
FROM AEA_APPLYINST_PROJ ahai123
	INNER JOIN AEA_PROJ_INFO proj ON proj.PROJ_INFO_ID = ahai123.PROJ_INFO_ID
	INNER JOIN AEA_HI_SERIESINST ahs ON ahs.APPLYINST_ID = ahai123.APPLYINST_ID
	LEFT JOIN AEA_PAR_STAGE stage ON ahs.STAGE_ID = stage.STAGE_ID
...
(SELECT ahai123.APPLYINST_PROJ_ID AS DFSJZJ, '000000' AS XZQHDM
	...
	END AS SBSJ, apt.THEME_CODE AS SPLCBM, NULL AS SPLCBBH, 1 AS SJYXBS, ahai123.CREATE_TIME AS MODIFY_TIME
FROM AEA_APPLYINST_PROJ ahai123
	INNER JOIN AEA_PROJ_INFO proj ON proj.PROJ_INFO_ID = ahai123.PROJ_INFO_ID
	INNER JOIN AEA_HI_SERIESINST ahs ON ahs.APPLYINST_ID = ahai123.APPLYINST_ID
	LEFT JOIN AEA_PAR_THEME apt ON apt.THEME_ID = proj.THEME_ID
WHERE ahs.STAGE_ID IS NULL);
...
(SELECT '000000' AS XZQHDM, lish_123.STATE_HIST_ID AS DFSJZJ
	, replace(proj.GCBM, '#', '-') AS GCDM
	, lish_123.ITEMINST_ID AS SPSXSLBM, lish_123.OPS_USER_NAME AS BLR
	, (
		SELECT org.ORG_NAME
...
FROM (
	SELECT DISTINCT hii.INOUTINST_ID AS DFSJZJ, '000000' AS XZQHDM, hii.ITEMINST_ID AS SPSXSLBM, hps.STAGEINST_ID AS BLSPSLBM, hps.APPLYINST_ID
...
FROM (
	...
	FROM (
		SELECT lish_123.STATE_HIST_ID AS DFSJZJ, '000000' AS XZQHDM
			, replace(proj.GCBM, '#', '-') AS GCDM
			, stageinst.stageinst_id AS SPSXSLBM
			, (
				SELECT org.ORG_NAME
				FROM opu_om_user_org ouo
					INNER JOIN opu_om_org org ON ouo.ORG_ID = org.org_id
				WHERE USER_ID = lish_123.OPS_USER_ID
				LIMIT 1
			) AS BLCS, lish_123.OPS_USER_NAME AS BLR, NEW_STATE AS BLZT
			, CASE
				WHEN IS_FLOW_TRIGGER = 1 THEN ifnull((
					SELECT ahc_123.MESSAGE_
					FROM ACT_HI_TASKINST aht_123
		...
		WHERE lish_123.NEW_STATE NOT IN ('11', '12', '13', '6', '7')
	...
	FROM (
		...
		FROM AEA_LOG_ITEM_STATE_HIST lish_123
			...
			INNER JOIN AEA_APPLYINST_PROJ aap ON aap.APPLYINST_ID = stageinst.APPLYINST_ID
			INNER JOIN AEA_PROJ_INFO proj ON proj.PROJ_INFO_ID = aap.PROJ_INFO_ID
		WHERE lish_123.NEW_STATE IN ('11', '12', '13')
			AND lish_123.NEW_STATE NOT IN ('6', '7')
