...
SELECT DFSJZJ, XZQHDM, SPLCBM, SPLCBBH, SPJDXH
	...
	, SPBMMC, SFLCBSX, SJYXBS, MODIFY_TIME, ITEM_ID
	, ITEM_BASIC_ID, STAGE_ID, APP_ID
FROM (
	SELECT psi.STAGE_ITEM_ID AS DFSJZJ, '000000' AS XZQHDM, apt_123.THEME_CODE AS SPLCBM, aptv_123.VER_NUM AS SPLCBBH
		...
		END AS BLJGSDFS
		, CASE
			WHEN psi.IS_OPTION_ITEM = '1' THEN aib_123.DUE_NUM
			ELSE NULL
		END AS SPSXBLSX
		, (
			SELECT ORG_CODE
		...
		END AS MODIFY_TIME, aib_123.ITEM_ID, aib_123.ITEM_BASIC_ID, stage.STAGE_ID, stage.APP_ID
...
SELECT stage.STAGE_ID AS DFSJZJ, '000000' AS XZQHDM, theme.THEME_CODE AS SPLCBM, theme.VER_NUM AS SPLCBBH, 5 AS SPJDXH
	...
	END AS MODIFY_TIME, NULL AS ITEM_ID, NULL AS ITEM_BASIC_ID, NULL AS STAGE_ID, NULL AS APP_ID
