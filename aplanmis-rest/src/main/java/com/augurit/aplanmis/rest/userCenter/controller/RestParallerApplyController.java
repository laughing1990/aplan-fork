...
import com.augurit.aplanmis.rest.userCenter.service.RestParallerApplyService;
import com.augurit.aplanmis.rest.userCenter.vo.ItemListVo;
import com.augurit.aplanmis.rest.userCenter.vo.MatListParamVo;
...
public class RestParallerApplyController {
    ...
    @ApiImplicitParams({@ApiImplicitParam(value = "阶段ID", name = "stageId", required = true, dataType = "string"),
            ...
            @ApiImplicitParam(value = "区域编码", name = "regionalism", dataType = "string")})
    public ContentResultForm<ItemListVo> listItemAndStateByStageId(@PathVariable("stageId") String stageId, @PathVariable("projInfoId") String projInfoId, @PathVariable("regionalism") String regionalism) {
        try {
            logger.error("-----listItemAndStateByStageId----start--------");
            long l = System.currentTimeMillis();
            ItemListVo vo = restParallerApplyService.listItemAndStateByStageId(stageId, projInfoId, regionalism);
            logger.error("-----listItemAndStateByStageId----end--------耗时：" + (System.currentTimeMillis() - l));
            return new ContentResultForm<>(true, vo);
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            return new ContentResultForm<>(false, null, "根据阶段id获取事项一单清列表数据异常");
        }
    }
    ...
    public ContentResultForm<ParallelApplyResultVo> startNetStageProcess(@RequestBody StageApplyDataPageVo stageApplyDataPageVo) {
        try {
            ...
            return new ContentResultForm<>(true, null, "申报成功!");
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
    ...
    public ContentResultForm<String> startInstApply(String applySource, String applySubject, String linkmanInfoId) {
        try {
            AeaHiApplyinst aeaHiApplyinst = aeaHiApplyinstService.createAeaHiApplyinst(applySource, applySubject, linkmanInfoId, "0", null, ApplyState.RECEIVE_UNAPPROVAL_APPLY.getValue(),"0",null);
    ...
    public ContentResultForm<Map<String, Object>> listItemByStateId(@PathVariable("stateId") String stateId, @PathVariable("stageId") String stageId, @PathVariable("regionalism") String regionalism, String projectAddress) {
        try {
            ...
            aeaItemBasicService.conversionBasicItemToSssx(itemList, regionalism, projectAddress, topOrgId);
            ...
            aeaItemBasicService.conversionBasicItemToSssx(coreItemList, regionalism, projectAddress, topOrgId);
            ...
            logger.error(e.getMessage(), e);
            return new ContentResultForm<>(false, null, "根据情形ID和阶段ID获取绑定事项及(阶段)子情形列表异常");
        }
    }
    ...
    public ContentResultForm<List<AeaParFactor>> listFactorByRootOrgId() {
        try {
            ...
            logger.error(e.getMessage(), e);
            return new ContentResultForm<>(false, null, "获取根因子列表数据异常");
        }
    }
    ...
    public ContentResultForm<List<AeaParFactor>> listChildFactorByParentFactorId(@PathVariable("factorId") String factorId) {
        try {
            ...
            logger.error(e.getMessage(), e);
            return new ContentResultForm<>(false, null, "根据父根因子获取子因子列表数据异常");
        }
    }
    ...
    public ContentResultForm<Map<String, Object>> getThemeByFactorIds(@PathVariable("factorIds") String[] factorIds) {
        try {
            ...
            logger.error(e.getMessage(), e);
            return new ContentResultForm<>(false, null, "根据选择的因子智能推荐主题异常");
        }
    }
}
