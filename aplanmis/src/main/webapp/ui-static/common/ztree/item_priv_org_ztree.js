var selectPrivOpsOmOrgTree;
//右边组织岗位用户树 配置setting信息
var privOpsOmOrgTreeSetting = {
    ...
    callback: {
        //双击事件
        onDblClick: onDblClickPrivOrgTreeNode
 ...
 * 打开组织树
 */
function selectPrivOpusOmOrgZtree(){

    var itemBasicFieldId = null;
    var itemBasicFieldName = null;

    $('#select_priv_opus_om_org_ztree_modal').modal('show');
    $('#select_priv_opus_om_org_ztree_modal_title').html('选择组织');
    $('#privOpusOmOrgDiv').animate({scrollTop: 0}, 800);//滚动到顶部
    showPrivOpusOmOrgTree();
    //初始化模糊搜索方法
    privFuzzySearch('privOpusOmOrgTree','#privOpusOmOrgZtreeKeyWord',null,true);
}
 ...
 * 关闭组织树
 */
function closeSelectPrivOpusOmOrgZtree() {

    $('#select_priv_opus_om_org_ztree_modal').modal('hide');
    showPrivOpusOmOrgTree();
}


//加载选择组织树
function showPrivOpusOmOrgTree() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data && data.length > 0) {
                var zNodes = data;
                selectPrivOpsOmOrgTree = $.fn.zTree.init($("#privOpusOmOrgTree"), privOpsOmOrgTreeSetting, zNodes);
            }
        }
    });
}
...
function onDblClickPrivOrgTreeNode(event, treeId, treeNode) {

    $("#selectPrivOrgBtn").trigger("click");
}

//折叠ztree树全部节点
function collapsePrivOpusOmOrgZtreeAllNode(treeId) {
...
//展开异步ztree树全部节点
function expandPrivOpusOmOrgZtreeAllNode(treeId) {

    var zTree = $.fn.zTree.getZTreeObj(treeId);
    expandPrivNodes(zTree.getNodes(),treeId);
}


//清空搜索左树文字框
function clearPrivOpusOmOrgZtreeForTree() {
...
//展开nods下全部子孙节点
function expandPrivNodes(nodes,treeId) {
 ...
 * @returns
 */
function privFuzzySearch(zTreeId, searchField, isHighLight, isExpand){
