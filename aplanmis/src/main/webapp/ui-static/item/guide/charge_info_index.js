...
function initValidate() {
    ...
    aedit_charge_info_validator = $("#aedit_charge_info_form").validate({
        ...
        submitHandler: function () {
            ...
            $.ajax({
                url: ctx+'/aea/item/guide/saveOrUpdateChargeInfo.do',
                type: 'POST',
                data: d,
                async: false,
...
function operatorFormatter(value, row, index, field) {
    ...
    var editBtn = '<a href="javascript:editChargeInfo(\'' + row.rowguid + '\')" ' +
                     'class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill" title="'+ title +'">' +
                     '<i class="'+ icoCss +'"></i>' +
                  '</a>';

    var delBtn  = '<a href="javascript:delChargeInfo(\'' + row.rowguid + '\')" ' +
...
function addChargeInfo(){
    ...
    if(curIsEditable){
        ...
        $('#aedit_charge_info_form input[name="rowguid"]').val('');
        $('#aedit_charge_info_form input[name="itemVerId"]').val(itemVerId);
        ...
        $.ajax({
            url: ctx+'/aea/item/guide/getChargeMaxSortNoByItemVerId.do',
            type: 'post',
            data: {'itemVerId': itemVerId},
            async: false,
            success: function (data) {
                if(data) {
                    $('#aedit_charge_info_form input[name="ordernum"]').val(data);
                }
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                swal('错误信息', XMLHttpRequest.responseText, 'error');
...
function editChargeInfo(id){
    ...
    if(curIsEditable){
        ...
        $.ajax({
            url: ctx+'/aea/item/guide/getGuideChargeInfoById.do',
            type: 'post',
            data: {'id': id},
            async: false,
            success: function (data) {
        ...
        if(id!=null&&id.length>0){
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'post',
                        url: ctx+'/aea/item/guide/delGuideChargeInfoById.do',
                        dataType: 'json',
                        data: {'id': id.toString()},
                        success: function (result) {
                            if (result.success) {
        ...
        if(rows!=null&&rows.length>0){
            ...
            swal({
                ...
                if (result.value) {
                    $.ajax({
                        type: 'post',
                        url: ctx+'/aea/item/guide/batchDelChargeInfoByIds.do',
                        dataType: 'json',
