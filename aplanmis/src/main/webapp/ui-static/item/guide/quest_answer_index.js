...
// function questAnswerResponseData(res) {
//
//     return res;
// }
...
function operatorFormatter(value, row, index, field) {
    ...
    var editBtn = '<a href="javascript:editQuestAnswer(\'' + row.id + '\')" ' +
                     'class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill" title="'+ title +'">' +
                     '<i class="'+ icoCss +'"></i>' +
                  '</a>';

    var delBtn  = '<a href="javascript:delQuestAnswer(\'' + row.id + '\')" ' +
...
function addQuestAnswer(){
    ...
    if(curIsEditable){
        ...
        $('#aedit_que_ans_form input[name="id"]').val('');
        $('#aedit_que_ans_form input[name="itemVerId"]').val(itemVerId);
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data) {
                    $('#aedit_que_ans_form input[name="ordernum"]').val(data);
                }
            },
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                swal('错误信息', XMLHttpRequest.responseText, 'error');
...
function editQuestAnswer(id){
    ...
    if(curIsEditable){
        ...
        $.ajax({
            url: ctx+'/aea/item/guide/getQuestAnswerById.do',
            type: 'post',
            data: {'id': id},
            async: false,
            success: function (data) {
