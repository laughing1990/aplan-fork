...
// 删除依据
function deleteWindowRelById(windItemId){
...
function batchDelete(curIsEditable){
    ...
    if(checkBoxs!=null&&checkBoxs.length>0){
        for(var i=0;i<checkBoxs.length;i++){
            ids.push(checkBoxs[i].windItemId);
        }
        swal({
            title: '提示信息',
            text: '您确定删除吗?',
...
function operatorFormatter(value, row, index, field) {
    ...
    var operatorStr = '';
    if(curIsEditable){
        operatorStr =  operatorStr + '<span><a href="javascript:deleteWindowRelById(\'' + row.windItemId + '\')" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="删除"><i class="la la-trash"></i></a></span>';
    }
    return operatorStr;
}
...
function showServiceWindow() {
    ...
    if(serviceWindowTable){
        windowQueryParams = [];
        windowQueryParams['itemVerId'] = itemVerId;
        ...
        ],ctx + '/aea/service/window/listItemUnselectedWindowByPage.do');
        windowQueryParams = [];
        windowQueryParams['itemVerId'] = itemVerId;
        serviceWindowTable.setQueryParams(windowQueryParams)
