...
agcloud.ui = {
    metronic : {
        ...
        showTip : function (message,type,time,closeWin,onCloseCallback) {
            var options = {
                message:message,
                time:time,
                type:type,
        ...
        showSwal : function (options) {
            ...
            if(options.onCloseCallback){
                swalOptions.onClose = options.onCloseCallback;
            }
            if(options.callback){
...
agcloud.utils = {
    ...
    dateTimeNoSecendFormat:function (val) {
        return dateFormatByExp(val, "yyyy-MM-dd hh:mm");
    },
    /**
...
/*            showMeridian:false, //在日期和小时选择界面，出现上下午的选项,默认false
...
//自定义组合序列化和去除字段值为‘’字符serializeJsonObject，
function serializeJsonObject(fn) {
    var json = {};
    var form = fn.serializeArray();
    $.each(form, function () {
        if(this.value!=''){
            if (json[this.name]) {
...
function openFullWindow(url){
    var parmw = "left=0,top=0,width="+screen.availWidth+",height="+screen.availHeight;
    parmw += ',fullscreen=no,toolbar=no,menubar=no,status=no,location=no,locationbar=no,directories=no,scrollbars=yes,resizable=yes';
    // var handle= window.open(url, 'newwindow', parmw);
    var handle= window.open(url);
    return handle;
}


agcloud.config = function () {
    var EditorWriteOption = {
        toolbars: [[
            'undo', 'redo', '|',
            ...
            'pagebreak','horizontal', 'date', 'time', 'spechars','|','searchreplace'
        ]],
    ...
    return {
        //ue编辑器可写的option
        EditorWriteOption:EditorWriteOption
        ////ue编辑器只读的option
        ,EditorReadOption:EditorReadOption
    }
}();