...
var leftLiActive=function (code) {
    // console.log('line203:'+code);
    //添加选中效果
    $.each($('#m_ver_menu ul li'),function (index,item) {
        // console.log($(item).find("a").data('code'));
        //如果code相等
...
var AleftLiActive=function (code) {
    // alert('onclick-tab');
    if(openedCode.indexOf(code)==-1)return;
    leftLiActive(code);
}
...
var openTabOrign=function () {
    ...
    if(url){
        ...
        if(index < 0){
            /!* onclick="leftLiActive(\''+code+'\');"*!/
            var tabItem = '<li class="nav-item m-tabs__item">\n' +
                '<a class="nav-link m-tabs__link" id="item_'
                +code
                +'" data-toggle="tab" href="#m_tabs_'
                +code
                +'" onclick="AleftLiActive(\''+code+'\');"  role="button" aria-haspopup="true" aria-expanded="false">'+title+'<i class="fa fa-times" onclick="ACloseTab(\''+code+'\')"></i></a>\n' +
                '</li>';
