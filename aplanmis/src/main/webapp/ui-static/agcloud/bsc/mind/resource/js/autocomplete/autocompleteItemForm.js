...
(function($) {
	$.fn.autocomplete = function(params) {
		...
		params = $.extend({
			...
			wordName: '',									//查找的字段的名称
            resultName:false,
			assistantName: false,							//辅助显示的字段的名称
            assistantCode: false,							//辅助显示的字段的名称
			resWordIsUpper: false,							//返回的字段名称需要大写， 默认否
		...
		this.each(function() {
				...
				switch(e.which) {
					...
					case 13: // Enter
						if(currentSelection > -1){
							...
							params.onSubmit(input.val(),data);
			...
			function transformPager(response){
                var page = {};
                if(response.count > 0){
                    page.currentPage = response.pageNum;
                    page.totalResult = response.count;
                    page.totalPage = pageCount(response.count, response.pageSize);
                }
                return page;
			}
					...
					if(true){
						...
						if(page.totalPage > 0){
							for(var i = 0; i < params.hints.length; i++){
								...
								//将要查询的字段放在检索显示li中
								//将相应的name存到全局数组中
								currentProposals.push(hints_data[params.resultName]);
								var element = $('<li></li>');
								var param_wordName = params.resultName;
								if(params.resWordIsUpper){
                                    param_wordName = param_wordName.toUpperCase();
								}
								var swn = hints_data[param_wordName];
								swn = swn.length > 40 ? swn.substr(0, 40) + '...' : swn;
								var element_xx = $('<span></span>')
									.html('<span style="overflow-x: hidden;" title="' + hints_data[param_wordName] + '">'+swn+'</span>')
                                ...
                                /*var pname = hints_data[params.assistantName] ? hints_data[params.assistantName] : '无'
                                var code = hints_data[params.assistantCode] ? hints_data[params.assistantCode] : '无'
                                element_xx.append('<span style="color: lightgrey;margin-left: 20px;" title="' + code + '">' + code + '</span>');
                                element_xx.append('<span style="color: lightgrey;margin-left: 20px;" title="' + pname + '">版本【' + pname + '】</span>');
*/
								if(params.showIcon){
									//如果头像为空的话，默认一个图片
						...
						if(1 == params.type){ //url获取数据
							...
							if(input.val() != ''){
									...
									if(params.hints[test].match(word)){
										...
										var element = $('<li></li>')
											...
											.click(function(){
												...
												proposalList.empty();
												params.onSubmit(input.val(),null);
												input.parents(".form-control").css({'z-index':888});
											})
											.mouseenter(function() {
												$(this).addClass('selected');
								...
								if(currentProposals.length > 0){
									var total_element = $('<span class="pull-right fr"></span>')
										.html('共查找到<label style="color:red;">' + currentProposals.length + '</label>条数据&nbsp;');
									page_element.append(total_element);
									var pnbtn_element = $('<span class="pull-left fl"></span>')
