            ...
            Text2Children: function(node, text) {
                ...
                function importChildren(node, children) {
                    for (var i = 0, l = children.length; i < l; i++) {
                        ...
                        childNode.data.name=childNode.data.text;
                        childNode.data.nodeTypeCode='situation';
                        childNode.data.operatorTag='new';
                ...
                if (this._dropSucceedTarget) {
                    ...
                    this._dragSources.forEach(function(source) {
                        ...
                        if(sourceNodeData.nodeTypeCode==AeaMindConst_MIND_NODE_TYPE_CODE_SITUATION){
                            ...
                            //是否通用
                            var isCommon='';
                            isCommon=sourceNodeData.extra.isCommon;
                            if(isCommon=='1'){
    ...
    value: function(require, exports, module) {
        ...
        Module.register("PriorityModule", function() {
            ...
            var PRIORITY_COLORS = [ null
                ,[ "#840023", "#840023" ] //  - red
                ,[ "#00c652", "#00c652" ] //  - green
                ,[ "#4720C4", "#4720C4" ] //  - purple
                ,[ "#B25000", "#B25000" ] //  - orange
                ,[ "#e03b3b", "#e03b3b" ] //  - blue

                ,[ "#f18842", "#f18842" ] // 6,7,8,9 - gray
                ,[ "#A3A3A3", "#515151" ]
                ,[ "#A3A3A3", "#515151" ]
                ,[ "#A3A3A3", "#515151" ]
            ];
                    ...
                    left: kity.createClass("PriorityRenderer", {
                        ...
                        update: function(icon, node, box) {
                            ...
                            if(data == 2) {
                                ...
                                var tempInfo='表';
                                if(nodeData.extra){
                                    if(nodeData.extra.formProperty){
                                        if(nodeData.extra.formProperty.indexOf('smart')>-1){
                                            tempInfo='智';
                                        }
                                    }
                                }
                                icon.setValue(tempInfo);
                            }
                            else {
                                icon.setValue('单');
                            }
...
_p[56] = {
    value: function(require, exports, module) {
        ...
        Module.register("ProgressModule", function() {
            ...
            // Designed by Akikonata
            // [MASK, BACK]
            var PROGRESS_COLORS = [ null
                ,[ "#FF1200", "#840023" ] //  - red
                ,[ "#00AF00", "#006300" ] //  - green
                ,[ "#A464FF", "#4720C4" ] //  - purple
                ,[ "#FF962E", "#B25000" ] //  - orange
                ,[ "#0074FF", "#01467F" ] //  - blue
                ,[ "#A3A3A3", "#515151" ] //  - gray
                ,[ "#A3A3A3", "#515151" ]
                ,[ "#A3A3A3", "#515151" ]
            ...
            var ProgressIcon = kity.createClass("ProgressIcon", {
                ...
                setValue: function(value) {
                    var back = this.back, mask = this.mask, number = this.number;
                    var color = PROGRESS_COLORS[value];
                    if (color) {
                        back.fill(color[1]);
                        mask.fill(color[0]);
                    }
         ...
         */
            var CameraCommand = kity.createClass("CameraCommand", {
                ...
                execute: function(km, focusNode) {
                    ...
                    var dx = 0, dy = viewport.center.y - offset.y; /*viewport.center.x - offset.x - offset.width / 2, dy = viewport.center.y - offset.y;*/
...
_p[79] = {
    ...
    _p[80] = {
        value: function (require, exports, module) {
            ...
            Module.register('LinkProcessStartModule', function () {
                ...
                // Designed by Akikonata
                // [MASK, BACK]
                var LINKPROCESSSTART_COLORS = [null, ['#ffc000', '#ffc000'], // 1 - red
    ...
    _p[81] = {
        value: function(require, exports, module) {
            ...
            Module.register('TerminateSituationModule', function() {
                var minder = this;

                // Designed by Akikonata
                // [MASK, BACK]
                // var TERMINATESITUATION_COLORS = [null, ['#FF1200', '#840023'], // 1 - red
                //     ['#0074FF', '#01467F'], // 2 - blue
                //     ['#00AF00', '#006300'], // 3 - green
                //     ['#FF962E', '#B25000'], // 4 - orange
                //     ['#A464FF', '#4720C4'], // 5 - purple
                ...
                var TERMINATESITUATION_COLORS = [null, ['#A3A3A3', '#A3A3A3'], // 1 - red
                    ['#0074FF', '#01467F'], // 2 - blue
                    ['#00AF00', '#006300'], // 3 - green
                    ['#FF962E', '#B25000'], // 4 - orange
                    ['#A464FF', '#4720C4'], // 5 - purple
                ...
                var TERMINATESITUATION_DATA = 'terminateSituation';

                // 进度图标的图形
                var TerminateSituationIcon = kity.createClass('TerminateSituationIcon', {
                    base: kity.Group,
                    ...
                    constructor: function() {
                        ...
                        this.setSize(20);
                        this.create();
                        this.setId(utils.uuid('node_terminateSituation'));
                    },
                    ...
                    setValue: function(value) {
                        ...
                        var color = TERMINATESITUATION_COLORS[value];
                 ...
                 *   -1: 当前没有选中的节点
                 */
                var TerminateSituationCommand = kity.createClass('SetTerminateSituationCommand', {
                    base: Command,
                    execute: function(km, value) {
                        var nodes = km.getSelectedNodes();
                        for (var i = 0; i < nodes.length; i++) {
                            nodes[i].setData(TERMINATESITUATION_DATA, value || null).render();
                        }
                        km.layout();
                    },
                    queryValue: function(km) {
                        ...
                        var val;
                        for (var i = 0; i < nodes.length; i++) {
                            val = nodes[i].getData(TERMINATESITUATION_DATA);
                            if (val) break;
                        }
                        return val || null;
                    },
                ...
                return {
                    'commands': {
                        'terminateSituation': TerminateSituationCommand
                    },
                    'renderers': {
                        left: kity.createClass('TerminateSituationRenderer', {
                            base: Renderer,

                            create: function(node) {
                                return new TerminateSituationIcon();
                            },

                            shouldRender: function(node) {
                                return node.getData(TERMINATESITUATION_DATA);
                            },

                            update: function(icon, node, box) {
                                var data = node.getData(TERMINATESITUATION_DATA);
                                var spaceLeft = node.getStyle('space-left'),
                                    x, y;
    ...
    _p[82] = {
        value: function(require, exports, module) {
            ...
            Module.register('SituationAnswerNumModule', function() {
                var minder = this;

                // Designed by Akikonata
                // [MASK, BACK]
                var SITUATIONANSWERNUM_COLORS = [null, ['#ffc000', '#ffc000'], // 1 - red
                    ['#006be5', '#006be5'], // 2 - blue
                    ['#00a000', '#00a000'], // 3 - green
                    ['#FF962E', '#B25000'], // 4 - orange
                    ['#A464FF', '#4720C4'], // 5 - purple
                    ['#A3A3A3', '#515151'], // 6,7,8,9 - gray
                    ['#A3A3A3', '#515151'],
                ...
                var SITUATIONANSWERNUM_DATA = 'situationAnswerNum';

                // 进度图标的图形
                var SituationAnswerNumIcon = kity.createClass('SituationAnswerNumIcon', {
                    base: kity.Group,
                    ...
                    constructor: function() {
                        ...
                        this.setSize(20);
                        this.create();
                        this.setId(utils.uuid('node_situationAnswerNum'));
                    },
                    ...
                    setValue: function(value) {
                        ...
                        var color = SITUATIONANSWERNUM_COLORS[value];
                 ...
                 *   -1: 当前没有选中的节点
                 */
                var SituationAnswerNumCommand = kity.createClass('SetSituationAnswerNumCommand', {
                    base: Command,
                    execute: function(km, value) {
                        var nodes = km.getSelectedNodes();
                        for (var i = 0; i < nodes.length; i++) {
                            nodes[i].setData(SITUATIONANSWERNUM_DATA, value || null).render();
                        }
                        km.layout();
                    },
                    queryValue: function(km) {
                        ...
                        var val;
                        for (var i = 0; i < nodes.length; i++) {
                            val = nodes[i].getData(SITUATIONANSWERNUM_DATA);
                            if (val) break;
                        }
                        return val || null;
                     },
                ...
                return {
                    'commands': {
                        'situationAnswerNum': SituationAnswerNumCommand
                    },
                    'renderers': {
                        left: kity.createClass('SituationAnswerNumRenderer', {
                            base: Renderer,

                            create: function(node) {
                                return new SituationAnswerNumIcon();
                            },

                            shouldRender: function(node) {
                                return node.getData(SITUATIONANSWERNUM_DATA);
                            },

                            update: function(icon, node, box) {
                                var data = node.getData(SITUATIONANSWERNUM_DATA);
                                var spaceLeft = node.getStyle('space-left'),
                                    x, y;
    ...
    _p[83] = {
        value: function(require, exports, module) {
            ...
            Module.register('NodeSelectModCmdModule', function() {
                var minder = this;
                var NodeSelectModCmdCommand = kity.createClass('SetNodeSelectModCmdCommand', {
                    base: Command,
                    execute: function(km, value) {
                        var nodes = km.getSelectedNodes();
                        for (var i = 0; i < nodes.length; i++) {
                            nodes[i].setData("", value || null).render();
                        }
                        km.layout();
                    },
                    queryValue: function(km) {
                ...
                return {
                    'commands': {
                        'nodeSelectModCmd': NodeSelectModCmdCommand
                    }
                };
            });
        }
