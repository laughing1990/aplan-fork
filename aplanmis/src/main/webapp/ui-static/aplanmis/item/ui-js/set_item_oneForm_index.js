...
function oneformIsActiveFormatter(value, row, index, field) {
    ...
    if(row.isActive=='1'){
        return  '<span class="m-switch m-switch--success">' +
            '  <label>' +
            '     <input type="checkbox" checked="checked" name="isActive" onclick="changeOneformIsActive(this,\''+ row.itemOneformId +'\',\''+ row.isActive +'\');">' +
            '     <span></span>' +
            '   </label>' +
            '</span>'
    }else{
        return  '<span class="m-switch m-switch--success">' +
            '  <label>' +
            '     <input type="checkbox" name="isActive" onclick="changeOneformIsActive(this,\''+ row.itemOneformId +'\',\''+ row.isActive +'\');">' +
            '     <span></span>' +
            '   </label>' +
            '</span>'
    }
...
function changeOneformIsActive(obj, id, isActive){
    ...
    if(id){
        if(isActive=='0'){
            $.ajax({
                url: ctx+'/aea/item/oneform/listActiveItemOneformNoPage.do',
                type: 'POST',
        ...
        swal({
            ...
            if (result.value){
                $.ajax({
                    url: ctx+'/aea/item/oneform/changIsActiveState.do',
                    type: 'POST',
                    data: {'id': id},
                    success: function (result) {
                        if(result.success){
