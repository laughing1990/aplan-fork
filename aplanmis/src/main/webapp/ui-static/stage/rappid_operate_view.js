...
$(function(){
    ...
    var cells = null;
    if(themeVerDiagram){
        cells =  JSON.parse(themeVerDiagram).cells;
    }
    if(projTree){
        var Main = {
            data() {
        ...
        new Ctor().$mount('#elTreeApp');
        proj[projInfoId] = JSON.parse(statusList);
    }
    var c = {min: .2, max: 5, step: .1};
    if(projName){
    ...
    // setTimeout("$('#uploadProgress').modal('hide');",550);
    ...
    paper.on('element:pointerdown', function(ele, size) {
        console.log('New cell with id ' + ele + ' added to the graph.'+size);
        return;
    })
    paper.on('element:pointerclick', function (a,b,c){
        alert();
        console.log(a,b,c);
    })
    paper.on('element:contextmenu', function (a,b,c){
        alert();
        console.log(a,b,c);
    })
    ...
    $('g.joint-cell.joint-type-bpmn.joint-type-bpmn-activity').on('mouseover',function(a,b){
        showItemDetail($(this));
    });
    $('g.joint-cell.joint-type-bpmn.joint-type-bpmn-activity').on('mouseleave', function(){
        $('.table-cell-hover').hide();
    })
});
...
function showItemDetail($this){
    var $itemTable = $('.table-cell-hover');
    ...
    var itemWidth = $this.find('div.content').width();
    var itemHeight = $this.find('div.content').height();
    var x, y, z;
    if(offset.top - tableHeight < 0){
        y = offset.top + itemHeight +20;
    }else{
        y = offset.top - tableHeight -20 ;
    }
    $itemTable.css('top', y);
    $itemTable.css('left', offset.left);

    $ps = $('#itemTableInfo p');
    $($ps[0]).find('span').text(itemInfo.itemName);
    $($ps[1]).find('span').text(itemInfo.orgName);
    $($ps[2]).find('span').text($this.find('div.content span:eq(1)').text());
    $($ps[3]).find('span').text(itemInfo.iteminstRunTime>0?itemInfo.iteminstRunTime:'0');
    $($ps[4]).find('span').text(itemInfo.iteminstStartTime?new Date(itemInfo.iteminstStartTime).format('yyyy-MM-dd'):'-');
    $($ps[5]).find('span').text(itemInfo.iteminstEndTime?new Date(itemInfo.iteminstEndTime).format('yyyy-MM-dd'):'-');
    $($ps[6]).find('span').text(itemInfo.statusName);
}

function loadItemDetail(){
    $.ajax({
        url:ctx + 'rest/user/item/schedule/diagram/'+projInfoId,
        method:'GET',
        success:function(res){
            if(res.success){
                debugger
                this.projItemDetail = res.content;
            }
        },error:function(res){
        ...
        $.each(statusList, function(ind, ele){
            ...
            if(ele.diagramItemList && ele.diagramItemList.length > 0){
                $.each(ele.diagramItemList, function(ind1, ele1){
                    ...
                    // setEleJsonAttr(itemIds, json, color,fontcolor, '特别程序', type, png);
                    changeViewItemProp(ele1.itemId, ele1.itemIds, fontcolor,null,color, png, ele.stageId, projId, cells, ele1);
...
/*    $.get(ctx+'/rest/project/diagram/status/json',{projInfoId:node.id}, function(res, a){
        if(res.success){
            proj[node.id] = res.content;
            console.log(JSON.stringify(proj));
            nodes.forEach(function(ele){
                setViewByProjResult(proj[ele.id], ele.id);
            })
        }else{
            if(res.message){
                swal('提示信息', res.message, 'info');
