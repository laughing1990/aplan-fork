...
function rightItemTreeClick(){
    debugger
    var treeObj = $.fn.zTree.getZTreeObj('rightExeItem');
    // 必选事项
    if (spanChecked == '0') {
        if (treeNode && treeNode.type == 'item') {
            if (treeNode.checked) {
...
// 修复事项重复
function handleStageItemsToOnly(isOptionItem){

    if(curIsEditable){

        var title = '此操作将去除重复事项数据,您确定要执行吗?';
        if(isOptionItem=='0'){
            title = '此操作将去除重复事项数据,并修复情形事项、材料事项数据,您确定要执行吗?';
        }
        swal({
            title: '此操作影响：',
            text: title,
            ...
            if (result.value) {
                $.ajax({
                    url: ctx + '/aea/par/stage/item/handleStageItemsToOnly.do',
                    type: 'POST',
                    data: {'stageId': currentBusiId, 'isOptionItem': isOptionItem},
                    async: false,
                    success: function (result) {
                        if (result.success) {
                            swal({
                            ...
                            if(isOptionItem=='0'){
                                setStageRelItems();
                            }else if(isOptionItem=='1'){
                                setStageRelNotNeedItems();
                            }else if(isOptionItem=='2'){
                                setStageRelFrontCheckItems();
                            }
                        }else{
