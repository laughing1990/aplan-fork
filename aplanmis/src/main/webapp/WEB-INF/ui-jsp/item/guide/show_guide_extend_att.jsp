...
<div id="show_guide_extend_att_modal" class="modal fade" tabindex="-1" role="dialog"
     aria-labelledby="show_guide_extend_att_modal_title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered  modal-lg" role="document" style="max-width:900px;">
        <div class="modal-content">
            <!-- 标题 -->
            <div class="modal-header" style="padding: 15px;height: 45px;">
                <h5 class="modal-title" id="show_guide_extend_att_modal_title">查看附件</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="closeAndRefreshData();">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" style="padding: 10px;">
                <div id="show_guide_extend_att_tb_scroll" style="height: 450px; overflow-y:auto;overflow-x:auto;">
                    <!-- 列表区域 -->
                    <div class="base" style="padding: 0px 5px;">
                        <table  id="show_guide_extend_att_tb"
                                data-toggle="table"
                                data-method="post"
                                ...
                                data-pagination-detail-h-align="left"
                                data-query-params="guideExtendAttTbParam"
                                data-response-handler="guideExtendAttTbResponseData"
...
<script type="text/javascript">
    ...
    function clearGuideExtendFile(){

        $("#resultSample").siblings('.custorm-style').find(".right-text").html("");
    ...
    var showGuideExtendType = null;
    function showGuideExtendAtt(type){

        $('#show_guide_extend_att_modal').modal('show');
        showGuideExtendType = type;
        searchGuideExtendAttSto(type);
    }

    function searchGuideExtendAttSto(type) {

        commonQueryParams = [];
        commonQueryParams.push({name: "tableName",value:"AEA_ITEM_GUIDE_EXTEND"});
        commonQueryParams.push({name: "pkName",value: type});
        commonQueryParams.push({name: "recordId",value: itemGuideExtendId});
        $("#show_guide_extend_att_tb").bootstrapTable('selectPage',1);  //跳转到第一页，防止其他页查询第一次不显示数据的问题。
        $("#show_guide_extend_att_tb").bootstrapTable('refresh');       //无参数刷新
    }

    function guideExtendAttTbParam(params){
    ...
    function guideExtendAttTbResponseData(res) {
        return res;
    }
    ...
    function attOperFormatter(value, row, index){
        ...
        if(row.attType!='KP'){
            var deleteBtn = '<a href="javascript:deleteGuideExtendAttrByDetailId(\'' + row.detailId + '\')" ' +
    ...
    function deleteGuideExtendAttrByDetailId(id){
            ...
            if(id){
                ...
                swal({
                    ...
                    if (result.value) {
                        $.ajax({
                            type: 'post',
                            url: ctx+'/aea/item/guide/extend/delItemGuideExtendAtt.do',
                            dataType: 'json',
                            data: {'detailId': id, 'type': showGuideExtendType, 'itemGuideExtendId': itemGuideExtendId},
                            success: function (result) {
                                if (result.success) {
                                    ...
                                    searchGuideExtendAttSto(showGuideExtendType);
                                } else {
    ...
    function closeAndRefreshData(){
        ...
        $('#show_guide_extend_att_modal').modal('hide');
