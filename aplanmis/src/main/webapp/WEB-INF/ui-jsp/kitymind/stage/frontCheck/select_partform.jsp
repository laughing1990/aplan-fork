...
<script type="text/javascript">
    ...
    function initSelectPartformTable() {
        select_partform_tb = defaultBootstrap("select_partform_tb",[{
            ...
            textAlign: 'center',
            formatter: function (value, row, index) {
                var btn =  '<a href="javascript:selectParFrontPartform(\'' + row.stagePartformId + '\',\''+row.partformName+'\',\''+row.isSmartForm+'\')" class="m-portlet__nav-link btn m-btn m-btn--hover-info m-btn--icon m-btn--icon-only m-btn--pill" title="选择"><i class="la la-plus"></i></a>';
                return btn;
            }
    ...
    function batchImprotParFrontPartform() {
    ...
    function batchSaveFrontPartform() {
        ...
        if(partforms.length==0){
            swal('错误信息', '没有勾选阶段扩展表单!', 'error');
        }else{
            var stagePartformIds = new Array();
            for(var i=0;i<partforms.length;i++){
                stagePartformIds.push(partforms[i].stagePartformId);
            }

            $("#uploadProgressMsg").html("批量导入阶段扩展表单，请稍后...");
            $("#uploadProgress").modal("show");

            $.ajax({
                url: ctx+'/aea/par/front/partform/batchSaveAeaParFrontPartform.do',
                type:'post',
                data:{stageId:currentBusiId,stagePartformIds:stagePartformIds.join(",")},
                async: false,
                dataType: 'json',
