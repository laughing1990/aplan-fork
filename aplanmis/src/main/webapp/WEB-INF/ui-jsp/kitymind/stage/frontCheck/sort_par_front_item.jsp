...
<!-- 分类排序 -->
<div id="sort_par_front_item_modal" class="modal fade" tabindex="-1" role="dialog"
     aria-labelledby="sort_par_front_item_modal_title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered  modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header" style="padding: 15px;height: 45px;">
                <h5 class="modal-title" id="sort_par_front_item_modal_title">事项信息前置检测排序</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" style="padding: 10px;">
                <div id="sortParFrontItemDiv" style="max-height:500px; overflow:auto;">
                    <div class="opusOmSortDiv">
                        <ul id="sortParFrontItemUl" class="block_ul_td"></ul>
                    </div>
                </div>
            </div>
            <div class="modal-footer" style="padding: 10px;height: 60px;">
                <button type="button" class="btn btn-secondary"
                        onclick="$('#sort_par_front_item_modal').modal('hide');">关闭</button>
            </div>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    $(function(){
        ...
        $('#sortParFrontItemDiv').niceScroll({
        ...
        Sortable.create(document.getElementById('sortParFrontItemUl'), {
            animation: 150,
            //拖拽完毕之后发生该事件
            onEnd: function (evt) {
                var liObjs = document.getElementsByName('sortParFrontItemLi');
                var ids = [];
                var sortNos = [];
                if(liObjs!=null&&liObjs.length>0) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: ctx+'/aea/par/front/item/updateAeaParFrontItemSortNos.do',
                        dataType: 'json',
                        data: {
                            'ids': ids.toString(),
