...
<!-- 分类排序 -->
<div id="stage_in_sort_modal" class="modal fade" tabindex="-1" role="dialog"
     aria-labelledby="stage_in_sort_modal_title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered  modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header" style="padding: 15px;height: 45px;">
                <h5 class="modal-title" id="stage_in_sort_modal_title">材料排序</h5>
            ...
            <div class="modal-body" style="padding: 10px;">
                <div id="opusOmSortDiv1" style="max-height:500px; overflow:auto;">
                    <div class="opusOmSortDiv">
                        <ul id="sortStageInUl" class="block_ul_td"></ul>
                    </div>
                </div>
            </div>
            <div class="modal-footer" style="padding: 10px;height: 60px;">
                <button type="button" class="btn btn-secondary" onclick="$('#stage_in_sort_modal').modal('hide');">关闭</button>
            </div>
        </div>
    </div>
</div>
<script type="text/javascript">
    $(function(){
        ...
        Sortable.create(document.getElementById('sortStageInUl'), {
            animation: 150,
            //拖拽完毕之后发生该事件
            onEnd: function (evt) {
                var liObjs = document.getElementsByName('sortStageInLi');
                var errorCount = 0;
                var inIds = [];
                var fileTypes = [];
                var sortNos = [];
                if(liObjs!=null&&liObjs.length>0) {
                    ...
                    $.ajax({
                        type: 'post',
                        url: ctx + '/aea/par/in/updateStageInSortNos.do',
                        dataType: 'json',
                        data: {
    ...
    //分类排序
    function sortStageIn(){

        $.post(ctx+'/aea/par/in/listStageInMatCertFormNoPage.do',{
            'stageId': currentBusiId
        }, function(data){
            if(data!=null&&data.length>0){

                $('#stage_in_sort_modal').modal('show');
                ...
                for(var i=0;i<data.length;i++){
                    ...
                    if(data[i].matProp=='m'){
                        name += '材料';
                    }else if(data[i].fileType=='c'){
                        name += '证照';
                    }else if(data[i].fileType=='f'){
                        name += '表单';
                    }
                    name += '】';
