...
<div id="show_service_wind_att_modal" class="modal fade" tabindex="-1" role="dialog"
     aria-labelledby="show_service_wind_att_modal_title" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered  modal-lg" role="document" style="max-width: 850px;">
        <div class="modal-content">
            <div class="modal-header" style="padding: 15px;height: 45px;">
                <h5 class="modal-title" id="show_service_wind_att_modal_title">查看附件</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true" onclick="closeAndReloadWindInfo();">&times;</span>
                </button>
            </div>
            <div class="modal-body" style="padding: 10px;">
                <div id="show_service_wind_att_tb_scroll" style="height: 450px; overflow-y:auto;overflow-x:auto;">
                    <!-- 列表区域 -->
                    <div class="base" style="padding: 0px 5px;">
                        <table  id="show_service_wind_att_tb"
                                data-toggle="table"
                                data-method="post"
                                ...
                                data-pagination-detail-h-align="left"
                                data-query-params="serviceWindAttTbParam"
                                data-response-handler="serviceWindAttTbResponseData"
            ...
            <div class="modal-footer" style="padding: 10px;height: 60px;text-align:right;">
                <button type="button" class="btn btn-secondary" onclick="closeAndReloadWindInfo();">关闭</button>
            </div>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    function clearServiceWindFile(){

        $("#mapAttFile").siblings('.custorm-style').find(".right-text").html("");
        $('#mapAttFileDiv').hide();
    }
    ...
    var attRelWindId = null;
    function showServiceWindAtt(){

        $('#show_service_wind_att_modal').modal('show');
        attRelWindId = $('#service_window_form input[name="windowId"]').val();
        if(attRelWindId){
            searchServiceWindAttSto();
        }else{
            swal('提示信息', '无法获取窗口id数据！', 'error');
        }
    }

    function searchServiceWindAttSto() {

        commonQueryParams = [];
        commonQueryParams.push({name: "tableName",value:"AEA_SERVICE_WINDOW"});
        commonQueryParams.push({name: "pkName",value: 'WINDOW_ID'});
        commonQueryParams.push({name: "recordId",value: attRelWindId});
        $("#show_service_wind_att_tb").bootstrapTable('selectPage',1);  //跳转到第一页，防止其他页查询第一次不显示数据的问题。
        $("#show_service_wind_att_tb").bootstrapTable('refresh');       //无参数刷新
    }

    function serviceWindAttTbParam(params){
    ...
    function serviceWindAttTbResponseData(res) {
        return res;
    }
    ...
    function attOperFormatter(value, row, index){
        ...
        if(row.attType!='KP'){
            var deleteBtn = '<a href="javascript:deleteServiceWindAttByDetailId(\'' + row.detailId + '\')" ' +
    ...
    function deleteServiceWindAttByDetailId(id){

        if(id){
            swal({
                title: '',
                text: '此操作将删除附件,确定要删除吗？',
                type: 'warning',
                ...
                if (result.value) {
                    $.ajax({
                        type: 'post',
                        url: ctx+'/aea/service/window/delServiceWindowAtt.do',
                        dataType: 'json',
                        data: {'detailId': id, 'windowId': attRelWindId},
    ...
    function closeAndReloadWindInfo() {

        $('#show_service_wind_att_modal').modal('hide');
        editItemServiceWindowById(attRelWindId);
    }
</script>
