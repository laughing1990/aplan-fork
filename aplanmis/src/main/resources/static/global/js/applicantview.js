...
var vm = new Vue({
  ...
  methods: {
    // 获取单位列表
    fetchApplicantList: function () {
      var ts = this;
      ts.loading = true;
      request('', {
        url: ctx + 'rest/applicant/listApplicants',
        type: 'get',
        data: ts.applicantSearchData
      }, function (res) {
        ts.loading = false;
        if (res.success) {
          ts.applicantList = res.content.list;
          ts.applicantTotal = res.content.total;
        } else {
    ...
    // 单位列表查询
    applicantListSearch: function () {
      this.applicantSearchData.pageNum = 1;
      this.fetchApplicantList();
    },
    // 单位列表清空
    applicantListReset: function () {
      this.applicantSearchData = JSON.parse(JSON.stringify(APPLICANTSEARCHDDATA));
      this.fetchApplicantList();
    },
    // 单位列表选中
    handleSelectionChange: function (list) {
      this.applicantSelectList = list;
    },
    // 单位列表换页
    handleCurrentChange: function (val) {
      this.applicantSearchData.pageNum = val;
      this.fetchApplicantList();
    },
    // 单位列表页面大小change
    handleSizeChange: function (val) {
      this.applicantSearchData.pageSize = val;
      this.fetchApplicantList();
    },
    // 单位列表批量删除
    batchDel: function () {
      var ts = this,
        _idList = [],
        _delData = {};
        if(!ts.applicantSelectList.length) return this.apiMessage('您尚未选中单位，请选择');
      confirmMsg('', '您确定删除选中单位吗？', function () {
        for (var i = 0; i < ts.applicantSelectList.length; i++) {
          _idList.push(ts.applicantSelectList[i].unitInfoId);
        }
        _delData.ids = _idList.join(',');
        ts.loading = true;
        request('', {
          url: ctx + 'rest/applicant/batchDeleteAeaApplicantByIds',
          type: 'get',
          data: _delData
        }, function (res) {
          ts.loading = false;
    ...
    // 获取单位证照类型
    // fectUnitLicenceTypeList: function () {
    //   var ts = this;
    //   ts.loading = true;
    //   request('', {
  ...
  created: function () {
    this.fetchApplicantList();
    //this.fectUnitLicenceTypeList();
    //this.fectUnitTypeList();
  },
});