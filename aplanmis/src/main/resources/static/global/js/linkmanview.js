...
var vm = new Vue({
  ...
  methods: {
    ...
    fetchLinkmanList: function () {
      ...
      ts.loading = true;
      request('', {
        url: ctx + 'rest/linkman/listAeaLinkMans',
        type: 'get',
        data: ts.linkmanSearchData
      }, function (res) {
        ts.loading = false;
        if (res.success) {
          ts.linkmanList = res.content.list;
          ts.linkmanTotal = res.content.total;
        } else {
          return ts.apiMessage('联系人列表获取失败！', 'error')
        }
      }, function () {
        ts.loading = false;
    ...
    linkManListSearch: function () {
      this.linkmanSearchData.pageNum = 1;
      this.fetchLinkmanList();
    },
    // 联系人列表清空
    linkmanListReset: function () {
      this.linkmanSearchData = JSON.parse(JSON.stringify(LINKMANSEARCHDDATA));
      this.fetchLinkmanList();
    },
    // 联系人列表选中
    ...
    handleCurrentChange: function (val) {
      this.linkmanSearchData.pageNum = val;
      this.fetchLinkmanList();
    },
    // 联系人列表页面大小change
    handleSizeChange: function (val) {
      this.linkmanSearchData.pageSize = val;
      this.fetchLinkmanList();
    },
    // 联系人列表批量删除
    batchDel: function () {
      var ts = this,
        _idList = [],
        _delData = {};
        if(!ts.linkmanSelectList.length) return this.apiMessage('您尚未选中联系人，请选择');
      confirmMsg('', '您确定删除选中联系人吗？', function () {
        for (var i = 0; i < ts.linkmanSelectList.length; i++) {
          _idList.push(ts.linkmanSelectList[i].linkmanInfoId);
        }
        _delData.ids = _idList.join(',');
        ts.loading = true;
        request('', {
          url: ctx + 'rest/linkman/batchDeleteAeaLinkMan',
          type: 'get',
          data: _delData
        }, function (res) {
          ts.loading = false;
