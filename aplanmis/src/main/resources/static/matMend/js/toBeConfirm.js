...
var pager = new Vue({
  ...
  methods: {
    ...
    fetchPageInfo: function () {
      ...
      // _getData.applyinstCorrectId = 'ad52ff89-1feb-4950-9016-d1c47a5d36be'
      ts.mloading = true;
      request('', {
          url: ctx + 'applyinst/correct/getItemCorrectRealIninst',
        type: 'get',
        data: _getData
      }, function (res) {
        ts.mloading = false;
    ...
    saveMaterialList: function (state) {
      ...
      this.$refs['materialListForm'].validate(function (valid, obj) {
        if (valid) {
          var _saveData = {};
          _saveData.correctId = ts.getSerachParamsForUrl('applyinstCorrectId');
          // _saveData.correctId = 'ad52ff89-1feb-4950-9016-d1c47a5d36be'
          ...
          ts.mloading = true;
          request('', {
              url: ctx + 'applyinst/correct/matCorrectConfirm',
            type: 'post',
            data: _saveData
          }, function (res) {
            ts.mloading = false;
    ...
    closeWindowTab: function () {
      ...
      ts.correctEndRefresh();
        // setTimeout(function () {
        //   window.parent.vm.removeTab(ts.getSerachParamsForUrl('currentTabId'));
        // }, 500)
    },
    // 如果是补正结束，那就刷新对应的列表页
    correctEndRefresh: function() {
      ...
      for (var i = 0; i < _iframs.length; i++) {
        ...
        parent.vm.activeTabIframe = ts.getSerachParamsForUrl('currentTabId');
        _tagIframe[0].contentWindow.location.reload(true);
        window.parent.vm.removeTab('dqr' + ts.getSerachParamsForUrl('applyinstCorrectId'));
    },
  },
  filters: {
