var vm = new Vue({
	...
	methods: {
		...
		fetchTableData: function () {
			...
			request('', {
				url: ctx + 'rest/conditional/query/listWaitUploadServiceResult',
				type: 'get',
				data: ts.searchFrom
			}, function (res) {
				ts.loading = false;
		...
		viewDetail: function (row) {
			var url = ctx + 'market/approve/approveSubmitMat.html?taskId=' + row.taskId + '&viewId=' + row.viewId + "&itemNature=8";
			if (row.busRecordId) {
		...
		signTask: function (event, row) {
			...
			request('bpmFrontUrl', {
				...
				url: ctx + 'market/approve/SignTaskOrUpdateAssign?taskId=' + taskId,
				type: 'get',
				data: {}
			}, function (result) {
				ts.loading = false;
				if (result.success) {
					...
					window.setTimeout(function () {
						window.open(ctx + 'market/approve/approveSubmitMat.html?taskId=' + taskId + '&viewId=' + viewId + '&busRecordId=' + busRecordId, '_blank');
