var mixins = {
  ...
  methods: {
    ...
    // 是否是小屏
    isSmallWinFn: function(){
      var _clientW = document.documentElement.clientWidth || document.body.clientWidth;
      if(_clientW <= 1306){
        this.isSmallWin = true;
      }else{
        this.isSmallWin = false;
      }
    },
...
// 获取昨日开始与结束日期
function culatorYesterDateRange() {
  var stdate = new Date()
  ...
  var start = stdate.getTime()
  var end = start + 24 * 60 * 60 -1;
  start = formatDate(new Date(start), 'yyyy-MM-dd')
