...
//augurit:完成填充颜色
var COMPLETED_ACTIVITY_FILL_COLOR = "#dddddd";
var CURRENT_ACTIVITY_FILL_COLOR = "#64e5a5";
...
function _showProcessDiagram(processInstanceId,isCheck) {
    checkParentProExist(processInstanceId);
    if (isCheck == "1") {
        //已办或办结时查看流程图
...
function _showChildrenProcessDiagram(procInstId,isCheck) {
    checkParentProExist(procInstId);
    if (isCheck == "1") {
        //已办或办结时查看流程图
        modelUrl = ctx + 'rest/process-instances/history/' + procInstId + '/model-json';
    } else {
        modelUrl = ctx + 'rest/process-instances/' + procInstId + '/model-json';
    }
    var request = jQuery.ajax({
        type: 'get',
        url: modelUrl + '?nocaching=' + new Date().getTime(),
...
function _showParentProcessDiagram(procInstId,isCheck) {
    checkParentProExist(procInstId);
    if (isCheck == "1") {
        //已办或办结时查看流程图
        modelUrl = ctx + 'rest/process-instances/history/' + procInstId + '/model-json';
            ...
            for (var i = 0; i < element.properties.length; i++) {
                ...
                if (element.properties[i].type && element.properties[i].type === 'list') {
                    ...
                    if(element.properties[i].value!=''){
                        ...
                        //augurit:如果name为空串时，直接显示value
                        if($.trim(propName)==''){
                            documentation += '<i>' + element.properties[i].value + '</i><br/>';
                        }else{
                            documentation += '<b>' + propName + '</b>: <i>' + element.properties[i].value + '</i><br/>';
                        }
                    }
